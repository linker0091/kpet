...
<div class="container">
	...
	<div class="consult_wrap clearfix">
		...
		<form action="/user/writeInsert" method="post" id="write_form" enctype="multipart/form-data">
		<div class="consult_writebox">
			<!-- 상담 글쓰기 페이지 -->
		     <div class="title_box">
		     	<ul class="clearfix">
		     		...
		     		<li>
						...
						<input type="text" name="cst_title" class="inputTypeText" >
		     ...
		     <div class="write_inner">
		     	<li>내용</li>
		     	<textarea id="cst_content" class="write_text" name="cst_content" rows="10" cols="80"></textarea>
		     </div>
		     <div class="title_box">
		     	<ul class="clearfix">
		     		<li>첨부파일</li>
		     		<li>
						<input type="file" name="cst_upload" id="cst_upload" class="file_input">
...
<%--$("input[type='file']").change(function(){});--%>
  ...
  if (input.files && input.files[0]) {
    ...
    $(".cst_btn").on("click", function(){
    	let img = $("#cst_upload").val();
    	console.log("클릭 " + img);
    	//return false;
    	if($('#cst_upload').get(0).files.length === 0)
    	{ 
    		// 파일을 선택하지 않았음 
    		$("#write_form").append("<input type='hidden' name='cst_img' value=''>");
    		console.log("선택안함");
    		
    	}--%>
    	<%--
    	if(img == ""){
    		$("write_form").append('$("input[name=' + cst_img + ']").val("")');
    		$("write_form").append('$("input[name=' + cst_uploadpath + ']").val("")');
    	}--%>
    	<%--$("#write_form").submit();
    });--%>
    //글등록 수정
