		...
		<div class="consult_box">
		     ...
		     <div class="consult_list clearfix">
	         	...
	         	<ul class="clearfix">
					<li>첨부</li>
		            <li id="cont_txt">
		            <c:if test="${cstInner.cst_img != null}">
						<img name="proudctImage" id="previewImage" src="/user/displayFile?fileName=<c:out value="${cstInner.cst_img }"></c:out>&uploadPath=<c:out value="${cstInner.cst_uploadpath }"></c:out>">
					</c:if>
					<c:if test="${cstInner.cst_img == null}">
						<li id="cont_txt" style="opacity:0.5;">첨부된 파일이 없습니다.</li>
					</c:if>
					</li>
	         	</ul>
	         </div>
			 ...
			 <button type="button" class="btnstyle cst_btn cst_modify_btn" data-cst_modify="${cstInner.cst_num }">수정</button>
			 <button type="button" class="btnstyle cst_remove_btn" data-cst_num="${cstInner.cst_num }">삭제</button>
			 </span>
	    </div> 
	</div>
...
<script>
	...
	$(".cst_modify_btn").on("click", function(){
		...
		actionForm.attr("action", "/user/cstModify");
		actionForm.submit();
	});
	//수정버튼
	<%--
	$(".cst_modify_btn").on("click", function(){
		let cst_num = $(this).data("cst_modify");
		location.href = "/user/cstModify?cst_num=" + cst_num;
	});
	--%>
	...
	$(".cst_remove_btn").on("click", function(){
		...
		location.href = "/user/cstRemove?cst_num=" + cst_num;		
	});
