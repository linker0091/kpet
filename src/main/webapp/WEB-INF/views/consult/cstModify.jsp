...
<body>
	...
	<div class="container">
		...
		<div class="consult_wrap clearfix">
			...
			<form action="/consult/cstModify" method="post" id="write_form"
					...
					<div class="title_box">
						<ul class="clearfix">
							...
							<li><select class="consult_category" name="cst_type">
								...
								value="${cstInner.cst_title }"> <input type="hidden"
								name="user_id" value="${cstInner.user_id }"> <input
								type="hidden" name="cst_num" value="${cstInner.cst_num }">
							</li>
						</ul>
					</div>
					<div class="write_inner">
						...
						<textarea id="cst_content" class="write_text" name="cst_content"
							rows="10" cols="80"><c:out
								value="${cstInner.cst_content }"></c:out></textarea>
					</div>
					<div class="title_box">
						<ul class="clearfix">
							<li>사진첨부</li>
							<li><input type="file" name="cst_upload" id="cst_upload"
								class="file_input" value="${cstInner.cst_upload }"> <!-- 이미지 변경시 기존이미지정보를 이용하여 기존이미지 삭제, 이미지 변경 안하면, 기존정보를 수정데이타로 사용  추가*-->
								<input type="hidden" name="cst_uploadpath"
								value="<c:out value='${cstInner.cst_uploadpath}' />"> <input
								type="hidden" name="cst_img"
								value="<c:out value='${cstInner.cst_img}'/>"> <label
								for="cst_upload">미리보기</label> <!-- 미리보기 코드 추가* --> <img
								name="cstImage" id="previewImage"
								src="/consult/displayFile?fileName=s_<c:out value='${cstInner.cst_img}'/>&uploadPath=<c:out value='${cstInner.cst_uploadpath}'/>">
							</li>
						</ul>
					</div>
				</div>
