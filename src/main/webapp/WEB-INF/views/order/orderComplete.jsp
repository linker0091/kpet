			...
			<div class="row">
				<div class="col-sm-12">
					...
					<table id="orderlist"
						...
						role="grid" width="90%" aria-describedby="example2_info">
						<thead>
							<tr role="row">
								<th>주문일</th>
								...
								<th class="sorting" tabindex="0" aria-controls="example2"
									rowspan="1" colspan="1"
									aria-label="Engine version: activate to sort column ascending">총 주문금액</th>
								<th class="sorting" tabindex="0" aria-controls="example2"
									rowspan="1" colspan="1"
						...
						<tbody>
								<tr role="row"
									...
									<td class="sorting_1"><fmt:formatDate
											value="${listVO.ord_regdate }"
											pattern="yyyy-MM-dd HH:mm" /></td>
									<td><a class="move"
										href="<c:out value="${listVO.ord_code }"></c:out>">
											${listVO.ord_code } </a></td>
									<td><c:out value="${listVO.ord_name }" />
									</td>
									<td>주문방법</td>
									<td><c:out value="${listVO.ord_price }"></c:out></td>
									<td class="cur_ord_state"><c:out value="${listVO.ord_state}" /></td>
									<td>
									<!-- 버튼* -->
									<button type="button" class="btn btn-outline-danger btn-sm" name="btnOrderCancel" data-ord_code="${listVO.ord_code }">취소</button>
									<button type="button" class="btn btn-outline-secondary btn-sm" name="btnOrderExchange" data-ord_code="${listVO.ord_code }">교환</button>
									<button type="button" class="btn btn-outline-secondary btn-sm" name="btnOrderReturn" data-ord_code="${listVO.ord_code }">반품</button>
									<input type="hidden" name="ord_state" value="${listVO.ord_state}">
								</tr>
...
<table class="table table-bordered table-hover dataTable ck"
	...
	<thead>
		...
		<tr role="row">
			...
			<th class="sorting2" tabindex="0" aria-controls="example2"
				rowspan="1" colspan="3"
				aria-label="Engine version: activate to sort column ascending">제품리뷰</th>
		</tr>
	</thead>
  ...
  <c:if test="${listVO.ord_code == oDetailVO.ord_code}">
	<tr role="row" class="<c:if test="${status.count % 2 == 0 }">odd</c:if><c:if test="${status.count % 2 != 0 }">even</c:if>">
		...
		<td>
			<c:out value="${oDetailVO.pro_name }" />
		</td>
		<td>
		...
		<td colspan="2"><c:out value="${oDetailVO.dt_ord_amount }"></c:out></td>
		<%--<td>
			<input type="button" name="btnOrderStateChange" value="취소" data-ord_code="${oDetailVO.ord_code }">
		</td>--%>
		<td rowspan="1" colspan="2">
		<%-- 
			<c:if test="${oDetailVO.ord_state eq '취소요청' || oDetailVO.ord_state ne '반품요청' && oDetailVO.ord_state ne '배송완료'}">
				<p class="state_txt">배송완료 전 입니다.</p>
			</c:if>	
	 	--%>
	 	<c:if test="${oDetailVO.ord_state ne '반품요청' && oDetailVO.ord_state ne '배송완료' && oDetailVO.ord_state ne '교환요청'}">
  			<p class="state_txt">배송완료 전 입니다.</p>
		</c:if>

			
  				<c:if test="${oDetailVO.ord_state eq '배송완료' || oDetailVO.ord_state eq '교환요청' || oDetailVO.ord_state eq '반품요청'}">
				<button type="button" class="review_btn btn btn-outline-info btn-sm" data-code="${oDetailVO.ord_code}" value="${oDetailVO.pro_num}">리뷰쓰기</button>
				</c:if>
							<c:forEach items="${reviewVO}" var="review">
							<!-- 수정* -->
 				 <c:if test="${review.ord_code == oDetailVO.ord_code && review.pro_num == oDetailVO.pro_num}">
				<button type="button" class="review_modify_btn btn btn-outline-success btn-sm" data-code="${oDetailVO.ord_code}" value="${oDetailVO.pro_num}">리뷰수정</button>							
			</c:if>
					</c:forEach>
		<%--<div></div> --%>
  ...
  <script>
	...
	//리뷰수정 클릭시    
  	...
  	$(function() {
  	  $('td:has(.review_btn):has(.review_modify_btn)').find('.review_btn').hide();
  	});
	...
	<!--
	//let cur_ord_state = $(this).parent('.row').find('.cur_ord_state').Html();
	//console.log("cur_ord_state: " + cur_ord_state);
	-->
	// 취소버튼 클릭시
    $('button[name="btnOrderCancel"]').on('click', function() {
    	
    	//현재 주문 상태
    	//let cur_ord_state = $(this).parent().find("input[name='cur_ord_state']").val();
    	
    	//let cur_ord_state = $(this).parent().find("td.cur_ord_state").text();
    	//console.log("cur_ord_state: " + cur_ord_state);
    	
    	let old_ord_state = $(this).siblings("input[name='ord_state']").val();
    	let new_ord_state = $(this).closest("tr").find("td.cur_ord_state");
		...
		if(old_ord_state == "배송완료" || old_ord_state == "교환요청" || old_ord_state == "반품요청" ){
		...
		$.ajax({
			...
			dataType: 'text',
			data:  {
				ord_code : ord_code , ord_state : ord_state
			},
			success: function(data){
				if(data == "success") {
					alert("주문 상품을 취소 요청 하였습니다.");
					new_ord_state.text("취소요청");
				}
			}
		});
    });
    ...
    $("button[name='btnOrderExchange']").on("click", function(){
    	...
    	console.log(old_ord_state);
    		...
    		if(old_ord_state != "배송완료" || old_ord_state == "취소요청"){
