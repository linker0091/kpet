...
#rew_paging { margin-bottom: 40px;}

</style>

		<div class="pro_review_header1 clearfix">
			<span> 리뷰   </span>
			<span>${ProductVO.pro_rewcount}</span><span> |</span>
			
				<span class="review_star"> ★ </span>
				<span>${ProductVO.pro_rewrating}</span>
		...
		<div class="product_review_media">
		 <c:if test="${reviewVO.rew_img  != null}">
			<span class="product_review_title"> 포토</span>
			<span class="product_review_title" style="font-weight: bold;">  </span>
			<div class="product_review_media_show_all">
				<span class="show_all"><a href="#" data-pronum="${ProductVO.pro_num}" class="all_img_btn"> 전체보기 ></a></span>
			</div>
		...
		<div class="product_review_media_sm">
			...
			<ul>
				<c:forEach items="${reviewImg }" var="review" varStatus="status">
					<li>
						<c:if test="${review.rew_img != null}">
						 	<img height="120" name="reviewImage" id="${review.rew_num}" data-pronum="${ProductVO.pro_num}" src="/review/displayFile?fileName=s_<c:out value="${review.rew_img }"></c:out>&uploadPath=<c:out value="${review.rew_uploadpath }"></c:out>">
						</c:if>
					</li>
				</c:forEach>
			</ul>
		...
		<input type="hidden" name="rew_num" value="${reviewVO.rew_num }">
								...
								<span class="review_star info_review_star"> <c:out value="${star }" /></span>
								<div class="product_review_content_right product_review_date">
									<fmt:formatDate value="${reviewVO.rew_regdate }" pattern="yyyy-MM-dd HH:mm" />	
								</div>
		...
		<br>
		<%--<span class="product_review_content_text">리뷰번호${reviewVO.rew_num }</span> --%>
		<span class="content_txt_innerbtn" data-rew_num="${reviewVO.rew_num }">리뷰 더보기</span>
		...
		<li>
		 <c:if test="${reviewVO.rew_img  != null}">
  		<img height="150" name="reviewImage" src="/review/displayFile?fileName=s_<c:out value="${reviewVO.rew_img }"></c:out>&uploadPath=<c:out value="${reviewVO.rew_uploadpath }"></c:out>">
  		</c:if>
		</li>
		</ul>
		</div>
		<div class="product_review_content_user">
		${fn:substring(reviewVO.user_id, 0, 4)  }***** <span>님의 리뷰 입니다</span>
		</div>
		</div>
		</c:forEach>
		...
		<div id="rew_paging">
			<nav aria-label="Page navigation example">
			  <ul class="pagination">
					...
					<li class='paginate_button previous ${rewPageMaker.prev ? "": "disabled" }'
						id="example2_previous"><a href="${rewPageMaker.startPage - 1}"
						aria-controls="example2" data-dt-idx="0" tabindex="0"><</a></li>
				</c:if>
				<c:forEach begin="${rewPageMaker.startPage }" end="${rewPageMaker.endPage }" var="num">	
					<li class='paginate_button ${rewPageMaker.cri.pageNum == num ? "active":"" }'><a href="${num}"
						aria-controls="example2" data-dt-idx="1" tabindex="0">${num}</a></li>
				</c:forEach>
				<c:if test="${rewPageMaker.next }">	
					<li class="paginate_button next" id="example2_next"><a
						href="${rewPageMaker.endPage + 1}" aria-controls="example2" data-dt-idx="7"
						tabindex="0">></a></li>
				</c:if>
			  </ul>
			</nav>
		...
		<form id="reviewForm" action="/review/productReview" method="get">
			<input type="hidden" id="rewPageNum" name="pageNum" value="${rewPageMaker.cri.pageNum}">
			<input type="hidden" id="rewAmount" name="amount" value="${rewPageMaker.cri.amount}">
		</form>
...
$(function() {
	  ...
	  $("#rew_paging").on("click", ".paginate_button a", function(e) {
	    e.preventDefault();
	    var rewPageNum = $(this).attr("href");
	    var rewAmount = $('#rewAmount').val();
