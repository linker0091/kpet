				...
				<div class="row">
					<div class="col-xs-12">
						<div class="box">
							...
							<div class="box-body">
								<div class="row">
									<div class="col-sm-1">진행상태</div>
									<div class="col-sm-11">
										<input type="button" value="전체" data-ordstate="전체"
											name="btnOrdState"> <input type="button"
											value="주문접수[${ordReceived}]" data-ordstate="주문접수"
											name="btnOrdState"> <input type="button"
											value="결제완료[${ordPaid}]" data-ordstate="결제완료"
											name="btnOrdState"> <input type="button"
											value="배송준비중[${ordPreparing}]" data-ordstate="배송준비중"
											name="btnOrdState"> <input type="button"
											value="배송중[${ordShipping}]" data-ordstate="배송중"
											name="btnOrdState"> <input type="button"
											value="배송완료[${ordDelivered}]" data-ordstate="배송완료"
											name="btnOrdState"> <input type="button"
											value="미입금취소[${unpaidCancel}]" data-ordstate="미입금취소"
											name="btnOrdState"> <input type="button"
											value="취소요청[${ordCancel}]" data-ordstate="취소요청"
											name="btnOrdState"> <input type="button"
											value="취소완료[${cancelCompleted}]" data-ordstate="취소완료"
											name="btnOrdState"> <input type="button"
											value="교환요청[${ordExchange}]" data-ordstate="교환요청"
											name="btnOrdState"> <input type="button"
											value="교환완료[${exchangeCompleted}]" data-ordstate="교환완료"
											name="btnOrdState"> <input type="button"
											value="반품요청[${ordReturn}]" data-ordstate="반품요청"
											name="btnOrdState"> <input type="button"
											value="반품완료[${returnCompleted}]" data-ordstate="반품완료"
											name="btnOrdState">
									</div>
								</div>
								<form id="searchForm" action="/admin/order/orderList"
									method="get">
							...
							<div class="box-body">
								<div id="example2_wrapper"
									...
									<div class="row">
										<div class="col-sm-6">
											<strong>총 주문건수 : ${ord_total }</strong>
										</div>
												...
												<tbody>
													<c:forEach items="${orderList }" var="orderVO"
														...
														<tr role="row"
															...
															<td><span><c:out value="${orderVO.ord_code }"></c:out></span>
															</td>
															<td><span><c:out value="${orderVO.ord_name }" /></span>
															</td>
															<td>주문방법</td>
															<td><span><c:out
																		value="${orderVO.ord_price }"></c:out></span></td>
															<td><select name="ord_state">
																	...
																	<option value="배송중"
																		<c:out value="${orderVO.ord_state eq '배송중'? 'selected':'' }" />>배송중</option>
																	<option value="배송완료"
																		<c:out value="${orderVO.ord_state eq '배송완료'? 'selected':'' }" />>배송완료</option>
																	<option value="미입금취소"
																		<c:out value="${orderVO.ord_state eq '미입금취소'? 'selected':'' }" />>미입금취소</option>
																	<option value="취소요청"
									...
									<div class="row">
										...
										<form id="actionForm" action="/admin/order/orderList"
											method="get">
									...
									<div class="row">
										<div class="col-sm-12">
											<input type="button" name="btnOrderStateAll"
												id="btnOrderStateAll" value="상태일괄변경"> <input
												type="button" id="btnCheckDelete" value="선택삭제"> <input
												type="button" id="btnExcelDownload" value="엑셀파일저장">
										</div>
									</div>
								</div>
							</div>
		...
		$(document)
				.ready(
						function() {
							...
							$(".paginate_button a")
									.on(
											...
											function(e) {
												...
												// 주문상태정보 추가
												actionForm
														.append("<input type='hidden' name='ord_state' value='${ord_state}'>");
							...
							//주문상태변경
							$("input[name='btnOrderStateChange']")
									.on(
											"click",
											function() {
									...
									.on(
											...
											function() {
												if ($(this).data("ordstate") == "전체") {
													...
													location.href = "/admin/order/orderStateList?ord_state="
															+ $(this).data(
																	"ordstate");
												}
											});
									...
									.on(
											...
											function() {
												...
												if (ord_state == null
													...
													location.href = "/admin/order/excelDown?startDate="
															+ startDate
															+ "&endDate="
															+ endDate
															+ "&type="
													...
													location.href = "/admin/order/excelDown?ord_state="
															+ ord_state;
												}
											});
