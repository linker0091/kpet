...
<div class="wrapper">
  ...
  <div class="content-wrapper">
	...
	<div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
		<div class="row">
			<div class="col-sm-6">총 관리자 : <c:out value="${admin_total}" />명</div>
			<div class="col-sm-6"></div>
		</div>
		<div class="row">
			<div class="col-sm-10" id="adminbox">
				<table id="adminlist" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
					<thead>
						<tr role="row">
							<th><input type="checkbox" id="checkAll" name="checkAll"></th>
							<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending">번호</th>
							<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending">아이디</th>
							<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">이름</th>
							<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">직책</th>
							<th class="sorting" tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending">마지막로그인</th>
					...
					<tbody>
						...
						<c:forEach items="${AdminList }" var="AdminVO" varStatus="status">
							<tr role="row"
								class="<c:if test="${status.count % 2 == 0 }">odd</c:if>
								<c:if test="${status.count % 2 != 0 }">even</c:if>">
								<td><input type="checkbox" class="check"
									value='<c:out value="${AdminVO.ad_id }"></c:out>'>
								</td>
								<td class="sorting_1"><c:out value="${status.count}" /></td>
								<td><c:out value="${AdminVO.ad_id }" />
								</td>
								<td><input type="text" value='<c:out value="${AdminVO.ad_name }" />'>
								</td>

								<td><select name="ad_position">
										<option value=""
											<c:out value="${AdminVO.ad_position == null? 'selected':'' }" />>관리자 직책 선택</option>
										<option value="신청중"
											<c:out value="${AdminVO.ad_position eq '신청중'? 'selected':'' }" />>신청중</option>
										<option value="총관리자"
											<c:out value="${AdminVO.ad_position eq '총관리자'? 'selected':'' }" />>총 관리자</option>
										<option value="중간관리자"
											<c:out value="${AdminVO.ad_position eq '중간관리자'? 'selected':'' }" />>중간 관리자</option>
										<option value="하위관리자"
											<c:out value="${AdminVO.ad_position eq '하위관리자'? 'selected':'' }" />>하위 관리자</option>
								</select></td>
...
<div class="dataTables_info" id="example2_info" role="status"
	...
	-->
		...
		<div class="row">
			<div class="col-sm-12">
				<input type="button" name="btnAdPositionAll" id="btnAdPositionAll" value="직책일괄변경">
