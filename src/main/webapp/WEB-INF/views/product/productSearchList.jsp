		        ...
		        <div class="item clarfix">
					...
					<select id="category" name="category">
					    ...
					    <c:forEach items="${mainCategory}" var="categoryVO">
					        <option value="${categoryVO.cate_code}" <c:if test="${categoryVO.cate_code eq con.cate_prtcode}">selected</c:if>>${categoryVO.cate_name}</option>
					    </c:forEach>
					</select>
					</div>
	            <div class="item clarfix">
	                <strong>상품검색</strong>            
					<input id="keyword" class="inputTypeText" size="15" type="text" value="<c:out value="${ con.keyword}" />">            
					</div>
				            
				<div class="item clarfix">
				<strong>판매가격대</strong> 
				<input id="price_min" class="input01" placeholder="" size="15" type="text" value="<c:out value="${ con.price_min != 0 ? con.price_min : ''}" />">
				 ~ 
				 <input id="price_max" class="input01" placeholder="" size="15" type="text" value="<c:out value="${ con.price_max != 0 ? con.price_max : ''}" />">
				</div>

				<div class="item clarfix">
	            <button id="search_btn" type="button" class="btnSubmitFix sizeM">검색</button>
	           	<button id="reset_btn" type="button" class="btnSubmitFix sizeM">검색 초기화</button>
				</div>
...
<p class="record">총 <strong>${pageMaker.total }</strong> 개 의 상품이 검색되었습니다.</p>
<ul class="orderby">
